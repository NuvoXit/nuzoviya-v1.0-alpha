...
def add_patient():
    try:
        ...
        new_patient = Patient(
            NIC=patient_id, first_name=first_name, last_name=last_name,
            DOB=DOB, address=address, telephone=telephone, email=email,
        )
...
def add_booking():
    try:
        ...
        if not all([first_name, last_name, telephone, patient_id, doctor_name, appointment_date, appointment_time]):
            return jsonify({"error": "Missing required fields"}), 400
